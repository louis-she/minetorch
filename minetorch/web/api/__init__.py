...
from minetorch.dockerize import docker_build
...
@experiment.route('/dockerimage', methods=['POST'])
def create_image(experiment_id):
    command = docker_build(g.experiment)
    return jsonify({'message': 'ok', 'command': command})
